...
/**
 * ASM MethodVisitor that replaces ChunkSavingSystems.tryQueueSync(...)
 * to skip saving already-on-disk chunks for shared instances.
 */
public class ChunkSavingTryQueueSyncMethodVisitor extends MethodVisitor {

    private final MethodVisitor target;

    public ChunkSavingTryQueueSyncMethodVisitor(MethodVisitor methodVisitor) {
    ...
    private void generateFixedMethod() {
        ...
        target.visitVarInsn(Opcodes.ISTORE, 4);

        // loopStart:
        target.visitLabel(loopStart);
        // if (index >= archetypeChunk.size()) goto loopEnd;
        target.visitVarInsn(Opcodes.ILOAD, 4);
        ...
        // if (!worldChunkComponent.getNeedsSaving()) goto continueLabel;
        ...
        // if (worldChunkComponent.isSaving()) goto continueLabel;
        target.visitVarInsn(Opcodes.ALOAD, 5);
        target.visitMethodInsn(
                Opcodes.INVOKEVIRTUAL,
                "com/hypixel/hytale/server/core/universe/world/chunk/WorldChunk",
        ...
        target.visitJumpInsn(Opcodes.IFNE, continueLabel);
        ...
        // worldChunkComponent.consumeNeedsSaving(); continue;
        target.visitVarInsn(Opcodes.ALOAD, 5);
        target.visitMethodInsn(
                Opcodes.INVOKEVIRTUAL,
                "com/hypixel/hytale/server/core/universe/world/chunk/WorldChunk",
        ...
        target.visitJumpInsn(Opcodes.GOTO, continueLabel);
        ...
        target.visitVarInsn(Opcodes.ASTORE, 9);

        // store.invoke(chunkRef, event);
        target.visitVarInsn(Opcodes.ALOAD, 2);
        target.visitVarInsn(Opcodes.ALOAD, 8);
        target.visitVarInsn(Opcodes.ALOAD, 9);
        target.visitMethodInsn(
                Opcodes.INVOKEVIRTUAL,
                "com/hypixel/hytale/component/Store",
                "invoke",
        ...
        // if (!event.isCancelled()) data.push(chunkRef);
        target.visitVarInsn(Opcodes.ALOAD, 9);
        target.visitMethodInsn(
                Opcodes.INVOKEVIRTUAL,

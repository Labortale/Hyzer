...
import com.hyzer.commands.CleanInteractionsCommand;
import com.hyzer.commands.CleanWarpsCommand;
import com.hyzer.commands.FixCounterCommand;
import com.hyzer.commands.InteractionStatusCommand;
import com.hyzer.commands.WhoCommand;
import com.hyzer.config.ConfigManager;
import com.hyzer.listeners.CraftingManagerSanitizer;
import com.hyzer.listeners.EmptyArchetypeSanitizer;
import com.hyzer.listeners.InteractionManagerSanitizer;
import com.hyzer.listeners.GatherObjectiveTaskSanitizer;
import com.hyzer.listeners.InstancePositionTracker;
import com.hyzer.listeners.ProcessingBenchSanitizer;
import com.hyzer.listeners.RespawnBlockSanitizer;
import com.hyzer.listeners.DefaultWorldRecoverySanitizer;
import com.hyzer.listeners.SharedInstanceBootUnloader;
import com.hyzer.listeners.SpawnBeaconSanitizer;
import com.hyzer.listeners.ChunkTrackerSanitizer;
import com.hyzer.optimization.ActiveChunkUnloader;
import com.hyzer.optimization.FluidFixerService;
import com.hyzer.optimization.PerPlayerHotRadiusService;
import com.hyzer.optimization.TpsAdjuster;
import com.hyzer.optimization.ViewRadiusAdjuster;
import com.hyzer.systems.InteractionChainMonitor;
import com.hyzer.systems.SharedInstancePersistenceSystem;
...
/**
 * Hyzer - Bug fixes for Hytale Early Access
 *
 ...
 */
public class Hyzer extends JavaPlugin {

    private static Hyzer instance;
    ...
    public Hyzer(@Nonnull JavaPluginInit init) {
        super(init);
        instance = this;
        getLogger().at(Level.INFO).log("Hyzer is loading...");
    }

    @Override
    protected void setup() {
        getLogger().at(Level.INFO).log("Setting up Hyzer...");

        // Initialize configuration first
        ConfigManager config = ConfigManager.getInstance();
        if (config.isLoadedFromFile()) {
            getLogger().at(Level.INFO).log("[CONFIG] Loaded configuration from mods/hyzer/config.json");
        } else {
        ...
        getLogger().at(Level.INFO).log("Hyzer setup complete!");
    }
    ...
    private void registerBugFixes() {
        ...
        getLogger().at(Level.INFO).log("[MON] InteractionChainMonitor registered - tracks Hyzer statistics");
    ...
    protected void start() {
        getLogger().at(Level.INFO).log("Hyzer has started! " + getFixCount() + " bug fix(es) active.");
    ...
    protected void shutdown() {
        getLogger().at(Level.INFO).log("Hyzer has been disabled.");
    ...
    public static Hyzer getInstance() {
        return instance;
    }
